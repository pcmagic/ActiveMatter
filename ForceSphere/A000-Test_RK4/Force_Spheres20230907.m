...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
rs2 = 12.0;
XaF0 = zeros(NS,NS);       YaF0 = zeros(NS,NS);
YbF0 = zeros(NS,NS);
XcF0 = zeros(NS,NS);       YcF0 = zeros(NS,NS);
for i = 1:NS
    for j = 1:NS
        if j == i
            continue
        end
        XaF0(i,j) = 1.5/rs2 - rs2^(-3);
        YaF0(i,j) = 0.75/rs2 + 0.5*rs2^(-3);
        YbF0(i,j) = -0.5*rs2^(-2);
        XcF0(i,j) = rs2^(-3);
        YcF0(i,j) = -0.5*rs2^(-3); 
    end
...
for i = 1:NS
    for j = 1:NS
        ...
        XaF(i,j) = 1.5/rss(i,j) - rss(i,j)^(-3) - XaF0(i,j);
        YaF(i,j) = 0.75/rss(i,j) + 0.5*rss(i,j)^(-3) - YaF0(i,j);
        YbF(i,j) = -0.5*rss(i,j)^(-2) - YbF0(i,j);
        XcF(i,j) = rss(i,j)^(-3) - XcF0(i,j);
        YcF(i,j) = -0.5*rss(i,j)^(-3) - YcF0(i,j); 
    end
end
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
